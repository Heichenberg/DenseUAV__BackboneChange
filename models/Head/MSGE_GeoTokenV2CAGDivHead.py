...
class MSGE_GeoTokenV2CAGDivHead(MSGE_GeoTokenV2CAGHead):
    ...
    def forward(self, x):
        if x.ndim != 4:
            raise ValueError(
                "MSGE_GeoTokenV2CAGDivHead expects [B, C, H, W] feature map, got {}".format(
                    tuple(x.shape)
                )
            )
        ...
        tokens, attn = self._make_tokens_with_attn(x_enhanced)
