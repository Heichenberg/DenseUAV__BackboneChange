...
class GeoTokenV2CAGKHead(nn.Module):
    def __init__(self, opt, num_queries):
        super().__init__()
        ...
        if x.ndim != 4:
            raise ValueError(
                "{} expects [B, C, H, W] feature map, got {}".format(
                    self.__class__.__name__, tuple(x.shape)
                )
            )
...
class GeoTokenV2CAG4Head(GeoTokenV2CAGKHead):
    def __init__(self, opt):
        super().__init__(opt, num_queries=4)


class GeoTokenV2CAG16Head(GeoTokenV2CAGKHead):
    def __init__(self, opt):
        super().__init__(opt, num_queries=16)
